...
void gibbs_beta_probit(Eigen::VectorXd& beta,
...
void compute_Qb_g_fc_probit(Eigen::SparseMatrix<double>& Q,
                     Eigen::VectorXd& b,
...
double l_lik_probit(Eigen::VectorXd Xbeta, Eigen::VectorXd Zg, Eigen::VectorXi y){
  Eigen::VectorXd probit_p = Xbeta + Zg;
  int n = y.size();
  Eigen::VectorXd p(n), ump(n);
  double out = 0;
  for(int j = 0; j < n; j++){
    p[j] = R::pnorm(probit_p[j], 0.0, 1.0, true, true);
    ump[j] = R::pnorm(probit_p[j], 0.0, 1.0, false, true);
    out += p[j] * y[j] + ump[j] * (1.0 - y[j]);
  }
  return(out);
}

double lfc_block_probit(Eigen::VectorXd gamma,
                     double s2g,
                     Eigen::VectorXd mu,
  ...
  lfc_joint = l_lik_probit(Xbeta, Zg, y) - (rank / 2.0) * std::log(s2g) -
...
//'
// [[Rcpp::export]]
Rcpp::List sample_bprobm(const Eigen::VectorXi y,
  ...
  for(int j=0; j<q ;j++){
    ...
    compute_Qb_g_fc_probit(Q_g_fc, b_g, ZtZ[j], K_g[j], Z[j], res_g, s2e, s2g_curr[j]);
  ...
  for(int k=0;k<niter;k++){
    ...
    ///////////////////////////////////////////////////////////////////////
    for(int j = 0; j < n ;j++){
      z_aux[j] = r_truncnorm(Xbeta[j] + Zg[j], 1.0, t_left[j], t_right[j]);
    }

    ///////////////////////////////////////////////////////////////////////
    // Sampling beta and s2b
    ///////////////////////////////////////////////////////////////////////
    gibbs_beta_probit(beta, Xbeta, XtX, X, Q_beta, s2e, z_aux - Zg);
    ...
    for(int j=0; j<q ;j++){
      ...
      l_fc_g_s2g_curr = lfc_block_probit(g_curr[j], s2g_curr[j],
                                      mu_g_fc, Q_g_fc, ldet_Q_g_fc, A[j], e[j], W,
                                      K_g[j], rank_K_g[j], pri_s2g[j], s2e, Xbeta, y, Z[j]);

      // Full conditionals parameters - prop
      compute_Qb_g_fc_probit(Q_g_fc, b_g, ZtZ[j], K_g[j], Z[j], res_g, s2e, s2g_prop);
      Chol_Q_g[j].factorize(Q_g_fc);
      mu_g_fc = Chol_Q_g[j].solve(b_g);
      ldet_Q_g_fc = 2.0 * Chol_Q_g[j].matrixL().toDense().diagonal().array().log().sum();
      ...
      l_fc_g_s2g_prop = lfc_block_probit(g_prop, s2g_prop,
                                      mu_g_fc, Q_g_fc, ldet_Q_g_fc, A[j], e[j], W,
                                      K_g[j], rank_K_g[j], pri_s2g[j], s2e, Xbeta, y, Z[j]);

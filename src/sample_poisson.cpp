...
Rcpp::List sample_poissonm_nob_auto(const Eigen::VectorXi y,//n ordinato
  ...
  std::vector<int>
    n_acc_s2g(q), Ta_s2g(q), n_acc_gamma(q);
  ...
  for(int k=0;k<niter;k++){
    ...
    metropolis_s2b(s2b, Q_beta, beta_curr, pri_s2b, FFb, n_acc_s2b);
    ...
    for(int j=0; j<q ;j++){
      ...
      metropolis_s2g_noblock(s2g_curr, K_g[j], g_curr[j], pri_s2g[j], rank_K_g[j], FFg[j], j, n_acc_s2g);
